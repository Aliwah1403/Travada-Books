...
import { Download01Icon, Mail01Icon, FileSpreadsheetIcon, Csv01Icon } from "@travada-books/ui/icons";
...
export function ExportTransactionsDialog({
  ...
  const label = selectedCount === 1 ? "1 transaction" : `${selectedCount} transactions`;
            ...
            <div className="grid grid-cols-2 gap-2">
              {(["csv", "xlsx"] as const).map((f) => (
                ...
                >
                  ...
                  <span className="font-medium">{f === "csv" ? "CSV" : "Excel (XLSX)"}</span>
                </button>
              ))}
            </div>
          </div>
