...
export function StatementDetailPage() {
  ...
  const statementUrl = `${window.location.origin}/st/${mockStatement.token}`
      ...
      <div className="flex shrink-0 items-center justify-between border-b px-6 py-3">
        <div className="flex items-center gap-3">
          <Button
            variant="ghost"
            size="icon-sm"
            onClick={() => navigate(`/customers/${mockStatement.customerId}`)}
          >
      ...
      <div className="flex-1 overflow-y-auto bg-muted/30">
        <div className="mx-auto flex max-w-2xl flex-col gap-4 px-4 py-8">
          ...
          <div className="rounded-lg border bg-background divide-y">
            <div className="px-5">
              <DetailRow label="Statement no." value={mockStatement.number} />
              <Separator />
              <DetailRow label="Period" value={`${mockStatement.dateFrom} – ${mockStatement.dateTo}`} />
              <Separator />
              <DetailRow
                label="Generated"
                value={format(mockStatement.generatedAt, "dd MMM yyyy, HH:mm")}
              />
              <Separator />
              <DetailRow label="Customer" value={mockStatement.customerName} />
            </div>
          </div>
        </div>
      </div>
