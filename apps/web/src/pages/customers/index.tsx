...
export function CustomersPage() {
  ...
  const totalOutstanding = Object.values(summaries).reduce(
    (sum, s) => sum + s.outstanding,
    0,
  );

  const topCustomerEntry = Object.entries(summaries).sort(
    (a, b) => b[1].totalPaid - a[1].totalPaid,
  )[0];
  const topCustomer =
    topCustomerEntry && topCustomerEntry[1].totalPaid > 0 ?
      {
        name:
          (customers ?? []).find((c) => c.id === topCustomerEntry[0])?.name ??
          "—",
  ...
  const stats = {
    ...
    totalOutstanding:
      Object.keys(summaries).length > 0 ? totalOutstanding : undefined,
            ...
            <table className='w-full caption-bottom text-xs min-w-[1820px]'>
              <TableHeader>
                <TableRow>
                  <TableHead
                    style={{ minWidth: 220 }}
                    className='h-12 px-4 text-xs sticky left-0 z-30 bg-background'
                  >
                    Customer
                  </TableHead>
                  <TableHead
                    style={{ minWidth: 220, left: 220 }}
                    className='h-12 px-4 text-xs sticky z-30 bg-background'
                  >
                    <div className='flex items-center justify-between gap-2'>
                      <span>Email</span>
                  ...
                  <TableHead
                    style={{ minWidth: 52 }}
                    className='h-12 w-13 sticky right-0 z-30 bg-background'
                  />
                </TableRow>
              </TableHeader>
              <TableBody>
                {filtered.map((customer) => (
                  ...
                  >
                    ...
                    <TableCell
                      style={{ left: 220 }}
                      className='py-3 text-xs text-muted-foreground sticky z-20 bg-background'
                    >
                      {customer.email ?? (
                    ...
                    <TableCell className='py-3 text-xs text-muted-foreground'>
                      {summaries[customer.id]?.invoiceCount ?? (
                        <span className='text-muted-foreground/50'>—</span>
                      )}
                    </TableCell>
                    <TableCell className='py-3 text-xs text-muted-foreground'>
                      {summaries[customer.id] ?
                        formatCurrency(
                          summaries[customer.id].totalPaid,
                          orgCurrency,
                        )
                      : <span className='text-muted-foreground/50'>—</span>}
                    </TableCell>
                    <TableCell className='py-3 text-xs'>
                      {summaries[customer.id] ?
                        <span
                          className={
                            summaries[customer.id].outstanding > 0 ?
                              "text-destructive font-medium"
                            : "text-muted-foreground"
                          }
                        >
                          {formatCurrency(
                            summaries[customer.id].outstanding,
                            orgCurrency,
                          )}
                        </span>
                      : <span className='text-muted-foreground/50'>—</span>}
                    ...
                    <TableCell
                      className='py-3 sticky right-0 z-20 bg-background'
                      onClick={(e) => e.stopPropagation()}
                    >
                      <CustomerActions
